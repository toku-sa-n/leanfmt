...
structure Options where private mk ::
...
private def OptionsBuilder.addStdin (builder : OptionsBuilder) : OptionsBuilder :=
  { builder with files_ := builder.files_.push "-", stdinCount_ := builder.stdinCount_ + 1 }
...
private def OptionsBuilder.parseArguments (args : Array String) :
    Except ValidationError OptionsBuilder := do
  return args.foldl processArg { }
where processArg (builder : OptionsBuilder) (arg : String) : OptionsBuilder :=
    if arg == "--help" || arg == "-h" then builder.showHelp
    else
      if arg == "--check" || arg == "-c" then builder.checkMode
      else
        if arg == "--in-place" || arg == "-i" then builder.inPlaceMode
        else if arg == "-" then builder.addStdin else builder.addFile arg
