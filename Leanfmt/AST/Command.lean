...
private inductive CommandImpl where
  ...
  deriving Inhabited
structure Command where private mk ::
...
private partial def findTypeSpec (stx : Syntax) : Option TypeSyntax :=
  ...
  | .node _ _ children => children.foldl (fun acc child => acc <|> findTypeSpec child) none
  | _ => none
...
private def parseExample (optDeclSig : Syntax) (body : Syntax) : Except FormatError Command := do
  ...
  | some type, some value =>
    return ⟨.example type value⟩
  | none, some _ =>
    throw (FormatError.unimplemented s! "example: unable to parse type from {optDeclSig.getKind}")
  | some _, none =>
    throw (FormatError.unimplemented s! "example: unable to parse value from {body.getKind}")
  | _, _ =>
    throw (FormatError.unimplemented s!"example: unable to parse type or value")
private def parseDeclaration (stx : Syntax) : Except FormatError Command := do
  ...
  | .node _ `Lean.Parser.Command.definition #[_, declId, optDeclSig, body, _] =>
    ...
    | .node _ `Lean.Parser.Command.declId #[id, _] =>
      return
        ⟨.definition (Identifier.fromName id.getId) (extractType optDeclSig) (extractValue body)⟩
    | _ =>
      throw (FormatError.unimplemented s! "definition declId: {declId.getKind}")
  ...
  | .node _ `Lean.Parser.Command.example #[_, declSig, declVal, _] =>
    parseExample declSig declVal
  | _ =>
    throw (FormatError.unimplemented s! "declaration kind: {stx.getKind}")
def Command.fromSyntax (stx : Syntax) : Except FormatError Command := do
  ...
  | _ =>
    throw (FormatError.unimplemented s! "command: {stx.getKind}")
end Leanfmt.AST
