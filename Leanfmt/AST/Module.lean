...
structure Header where private mk ::
  private prelude : Bool
  private imports : Array Lean.Name
  deriving Inhabited
structure Module where private mk ::
...
private partial def extractImportName (stx : Lean.Syntax) : Option Lean.Name :=
  ...
  | .node _ _ args => args.foldl (fun acc arg => acc <|> extractImportName arg) none
  | _ => none
private partial def collectImports (stx : Lean.Syntax) (acc : Array Lean.Name) : Array Lean.Name :=
  ...
  | .node _ `Lean.Parser.Module.import _ =>
    ...
    | none => acc
  | .node _ _ args => args.foldl (fun acc arg => collectImports arg acc) acc
  | _ => acc
...
private def mkCoreContext (fileName : String) (fileMap : Lean.FileMap) : Lean.Core.Context :=
  ...
  { fileName := fileName, fileMap := fileMap, options := options, currRecDepth := 0,
    maxRecDepth := 1000, ref := Lean.Syntax.missing, currNamespace := Lean.Name.anonymous,
    openDecls := [], initHeartbeats := 0, maxHeartbeats := Lean.Core.getMaxHeartbeats options,
    currMacroScope := Lean.firstFrontendMacroScope, diag := false, cancelTk? := none,
    suppressElabErrors := false, inheritedTraceOptions := ∅ }
private def mkCoreState (env : Lean.Environment) : Lean.Core.State :=
  { env := env, nextMacroScope := Lean.firstFrontendMacroScope + 1, ngen := { }, auxDeclNGen := { },
    traceState := { }, cache := { }, messages := { }, infoState := { }, snapshotTasks := #[] }
private def prettyCommand (cmd : Lean.Command) (ctx : Lean.Core.Context) (st : Lean.Core.State)
  ...
  format
    ...
    let env ←
      liftM <| do
          ...
          Lean.importModules (imports := #[{ module := `Lean }, { module := `Std }]) (opts := { })
              (loadExts := true)
...
partial def Module.parse (source : String) (fileName : String) : IO Module := do
  ...
  let env ←
    importModules (imports := #[{ module := `Lean }, { module := `Std }]) (opts := { }) (loadExts :=
        true)
  ...
  return
    { fileName := fileName, fileMap := inputCtx.fileMap, header := header, commands := commands }
end Leanfmt.AST
