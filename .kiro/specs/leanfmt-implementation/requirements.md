...
4. When 引数が一切指定されないとき, the Leanfmt CLI shall 標準入力を整形対象として読み込む
5. When `-` を複数回指定したとき, the Leanfmt CLI shall 入力指定の誤りとしてエラー終了する
...
5. When `leanfmt --check` がファイル指定なしで実行されたとき, the Leanfmt CLI shall `leanfmt` と同様に標準入力を読み取り、一致判定を行う
6. When stdout モードで複数ファイルを整形する場合, the Leanfmt CLI shall 各ファイルの出力前に `foo.lean` の形式でファイル名を 1 行出力し、その直後に空行を 1 行挿入する（単一ファイルまたは stdin の場合はヘッダを出力しない）
7. When `leanfmt --check` がファイル指定なしで TTY 上で実行されたとき, the Leanfmt CLI shall `leanfmt` と同様に標準入力を待機して読み取り、一致判定を行う
...
3. While 入力が Lean 4 の構文として有効であるとき, the Leanfmt CLI shall 各入力ソースごとに構文的に有効な出力を生成する
4. When 複数の入力ソースを一括で処理して標準出力に連結する場合, the Leanfmt CLI の連結結果は構文的に有効であるとは限らない
5. The Leanfmt CLI shall 少なくとも leanfmt 自身の実装ソースコードを整形できる
...
#### Acceptance Criteria
1. When check モードで任意の入力が整形結果と一致しないとき, the Leanfmt CLI shall 不一致を示す結果を報告し非成功の終了ステータスを返す
2. When check モードで全ての入力が整形結果と一致するとき, the Leanfmt CLI shall 一致を示す結果を報告し成功の終了ステータスを返す
